...
class AiAssessmentController extends ControllerBase {
  ...
  public static function create(ContainerInterface $container): static {
    return new static(
      ...
      $container->get('ai_content_audit_scoring.technical_audit'),
   ...
   *
   * @param \Drupal\ai_content_audit_scoring\Entity\AiContentAssessment $ai_content_assessment
  ...
  public function report(AiContentAssessment $ai_content_assessment): array {
    ...
    return [
      ...
      '#attached' => [
        'library' => ['ai_content_audit_scoring/assessment-report'],
      ],
      '#cache' => [
  ...
  private function buildScoreHistory(?NodeInterface $node, int $current_id): array {
    ...
    $history = [];
    if (!empty($ids)) {
      /** @var \Drupal\ai_content_audit_scoring\Entity\AiContentAssessment $hist */
